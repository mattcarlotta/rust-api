...
type ResponseVec = Custom<Vec<u8>>;

// API endpoint: GET http://domain.com/image/example.png?width=800
// Try visiting:
//   http://127.0.0.1:5000/placeholder <-- is this possible?
...
async fn serve_image(
  ...
  // store original pathname if no width query: <rootdir>/<filename>_<width>.<ext>
  // or store pathname with width: <rootdir>/<filename>_<width>.<ext>
  let requested_fp = match parsed_width == 0 {
    true => get_string_path(&fn_path),
    false => {
      let image_path = get_string_path(&fn_path);
